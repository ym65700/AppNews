...
public class LocalCacheUtils {
    ...
    public void putBitmap(String imageUrl, Bitmap bitmap) {
        ...
        //判断sdcard是否挂载
        if(Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
            //保存图片在/mnt/sdcard/xinwen/http://192.168.21.165:8080/xsxxxx.png
            //保存图片在/mnt/sdcard/xinwen/llkskljskljklsjklsllsl
            try {
                String fileName = MD5Encoder.encode(imageUrl);//llkskljskljklsjklsllsl
                ///mnt/sdcard/xinwen/llkskljskljklsjklsllsl
                File file = new File(Environment.getExternalStorageDirectory()+"/xinwen",fileName);
                File parentFile =  file.getParentFile();//mnt/sdcard/xinwen
